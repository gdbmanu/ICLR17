...
Optimizing foveal/multi-view image inspection with active vision has been addressed for quite a while in computer vision. Direct policy learning from gradient descent was e.g. proposed in 1991 by \cite{schmidhuber1991learning} using BPTT through a pre-processed generative model. The simplified "pyramidal" focal encoding of images is found in \cite{kortum1996implementation,wang2003foveation} and \cite{chen2017eccentricity}. The embedding of active vision in a Bayesian/POMDP evidence accumulation framework dates back from \cite{bajcsy1988active}, with a more formal elaboration in \cite{najemnik2005optimal} and \cite{butko2010infomax}. 
%hmad2013active,potthast2016active}
Low-level saliency maps in saccade modeling date back from \cite{itti2001computational}, with many follow-ups in image/video compression (see \cite{wang2003foveation}).
Considering the Free Energy/minimum description length setup for processing optimization in deep learning was recently proposed in \cite{louizos2017bayesian}.
